...
describe('BalanceService', () => {
  ...
  describe('getBalance', () => {
    test('正常系: 未精算取引がない場合は0を返す', async () => {
      ...
      const balance = await balanceService.getBalance(1, 2);
      expect(balance).toEqual({ amount: 0 });
    });
    ...
    test('正常系: 自分が支払った取引の場合', async () => {
      ...
      const balance = await balanceService.getBalance(1, 2);
      expect(balance).toEqual({ amount: 500 }); // 相手の負担額を受け取る
    });
    ...
    test('正常系: 相手が支払った取引の場合', async () => {
      ...
      const balance = await balanceService.getBalance(1, 2);
      expect(balance).toEqual({ amount: -500 }); // 自分の負担額を支払う
    });
    ...
    test('正常系: 複数の取引がある場合', async () => {
      ...
      const balance = await balanceService.getBalance(1, 2);
      expect(balance).toEqual({ amount: 200 }); // 500 - 300
    });
    ...
    test('異常系: ユーザーが存在しない場合', async () => {
      ...
      await expect(balanceService.getBalance(1, 2)).rejects.toThrow('User not found');
    });
    ...
    test('異常系: 自分自身との精算を試みた場合', async () => {
      ...
      await expect(balanceService.getBalance(1, 1)).rejects.toThrow(
        'Cannot calculate balance with yourself'
      );
    });
  });
});
